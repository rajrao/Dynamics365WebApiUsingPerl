...
my $c = new CrmWebApiAccessor('crmUrl' => $crmUrl, 'crmApiUrl' => $crmApiUrl, 'clientId' => $clientId,  'clientSecret' => $clientSecret, 'debug' => 1);
...
my (@data) = $c->getData('accounts?$select=accountid,name&$top=20');
